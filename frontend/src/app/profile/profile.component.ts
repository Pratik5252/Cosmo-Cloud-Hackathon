...
export class ProfileComponent implements OnInit {
	...
	initializeUserProfile() {
		if (this.userProfile != null) {
			...
			this.userProfile.preferredLearningMethods = this.userProfile.preferredLearningMethods || [];
			this.userProfile.currentSkills = this.userProfile.currentSkills || [];
			this.userProfile.preferredContentTypes = this.userProfile.preferredContentTypes || [];
			this.userProfile.location = this.userProfile.location || { country: '', city: '' };
			this.userProfile.timeCommitment = this.userProfile.timeCommitment || { weeklyHours: '0', preferredDays: [] };
		}
	}
	...
	async loadUserProfile(): Promise<void> {
		...
		this.userService.getUser(this.sampleUserId)
			.then((response) => {
				this.isLoading = false;
				this.errorMessage = '';
				this.userProfile = response;
				if (response === null) {
					this.errorMessage = 'We are facing some issues loading your profile.  Please reload the window';
				}
	...
	async updateUser(): Promise<void> {
		...
		if (this.userProfile) {
			...
			if (this.userProfile.careerGoals?.length) {
				this.userProfile.careerGoals.forEach((goal: { skillsToDevelop: any; }) => {
			...
			this.userService.updateUser(this.sampleUserId, userProfileToUpdate)
				.then((response) => {
